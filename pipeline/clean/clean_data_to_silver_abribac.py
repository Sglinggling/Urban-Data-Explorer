"""
Nettoyage des données PAVDA (abribacs à déchets alimentaires) en couche Silver.
Harmonise les colonnes, extrait l'arrondissement parisien et les coordonnées géographiques.
Entrée : data/bronze/abribac_dechets_alimentaires.csv  →  Sortie : data/silver/abribac_dechets_alimentaires.csv
"""
...
# Extrait (longitude, latitude) depuis une chaîne au format "lat, lon"
...
# Extrait (longitude, latitude) depuis un GeoJSON sérialisé, avec fallback regex
...
def clean_dechets_silver(
    ...
    # Normalise le numéro d'arrondissement : supporte les formats code INSEE (751xx),
    # code postal (750xx) et les libellés textuels ("5e", "5ème", "05", etc.)
    arr_num = None
    ...
    # Coordonnées issues de geo_point_2d en priorité, complétées par geo_shape si manquantes
